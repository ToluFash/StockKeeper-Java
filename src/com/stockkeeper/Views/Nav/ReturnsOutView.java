...
public class ReturnsOutView extends PrimaryViewRR implements MouseListener, ActionListener, DocumentListener, AdjustmentListener {
    //Core Models
    private Entity entity;
    private User user;

    public ReturnsOutView(Entity entity, User user)
    {
        this.entity = entity;
        this.user = user;
        ...
        initJMenu();
        initTopPanels();
        initContentPanel("Returns Outward", "Supplier");
        initDetailsPanel();
        setUpTimer(Global.returnsOut);
        initModels((Date)datePickerTicket.getModel().getValue(), Global.returnsOut);
        initGraphs(Global.returnsOut);
        setChartViewLine(Global.returnsOut.getDistribution((Date) datePickerTicket.getModel().getValue()));
        displayModel();
    ...
    public void mouseClicked(MouseEvent e) {
        mouseClickedRR(e, "Returns Outward");
    }
    @Override
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        initModels((Date) d1.getValue(), Global.returnsOut);
        ...
        lineGraphPanel.refreshGraph(Global.returnsOut.getDistribution((Date)datePanelTicket.getModel().getValue()),0);
        ...
        if (isListView()){
            ...
            setChartViewBar(Global.returnsOut.getEntriesbyProductsModelGraph((Date) datePickerTicket.getModel().getValue()));
            detailsCon.setDetailsVisible(false);
        }

        if (e.getSource() == openP){
            new TicketView(Global.mainView, inContext.getTicket().getId(),"Returns Outward", inContext.getTicket()).setVisible(true);
        }
        if (e.getSource() == openR){
            new TicketView(Global.mainView, ((inContext.getTicket())).getReference(),"Purchases", Global.purchases.getTicket(((inContext.getTicket())).getReference())).setVisible(true);
        }
    }
