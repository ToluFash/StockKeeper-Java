...
public class PurchasesView extends PrimaryViewSP {
    //Core Models
    private Entity entity;
    private User user;


    public PurchasesView(Entity entity, User user)
    {
        this.entity = entity;
        this.user = user;
        setLayout(new BorderLayout());
        initTopPanels();
        initContentPanel("Purchases", "Supplier");
        initDetailsPanel();
        setUpTimer(Global.purchases);
        initModels((Date)datePickerTicket.getModel().getValue(), Global.purchases);
        initGraphs(Global.purchases);
    ...
    private void initDetailsPanel(){
        ...
        detailsCon = new PurchasesPanel(productLabel, qtyLabel, ticketIDLabel,ticketEntrySizeLabel,customerLabel,dateLabel);
        detailsCon.setDetailsVisible(false);
        //Nav Area
        //Loading Components
        detailsPanel.add(infoBar, BorderLayout.NORTH);
        detailsPanel.add((PurchasesPanel)detailsCon, BorderLayout.CENTER);
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        initModels((Date) d1.getValue(), Global.purchases);
