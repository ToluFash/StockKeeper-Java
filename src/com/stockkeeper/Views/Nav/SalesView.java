...
public class SalesView extends PrimaryViewSP {
    //Core Models
    private Entity entity;
    private User user;

    public SalesView(Entity entity, User user)
    {
        this.entity = entity;
        this.user = user;
        setLayout(new BorderLayout());
        initTopPanels();
        initContentPanel("Sales", "Supplier");
        initDetailsPanel();
        setUpTimer(Global.sales);
        initModels((Date)datePickerTicket.getModel().getValue(), Global.sales);
        initGraphs(Global.sales);
        setChartViewLine(Global.sales.getDistribution((Date) datePickerTicket.getModel().getValue()));
        displayModel();
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        initModels((Date) d1.getValue(), Global.sales);
        ...
        lineGraphPanel.refreshGraph(Global.sales.getDistribution((Date)datePanelTicket.getModel().getValue()),0);
        ...
        if (isListView()){
            ...
            setChartViewBar(Global.sales.getEntriesbyProductsModelGraph((Date) datePickerTicket.getModel().getValue()));
            detailsCon.setDetailsVisible(false);
        }
    }
