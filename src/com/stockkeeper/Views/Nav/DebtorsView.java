...
public class DebtorsView extends PrimaryViewSP implements MouseListener, ActionListener, DocumentListener, AdjustmentListener {
    //Core Models
    ...
    public DebtorsView(Entity entity, User user)
    {
        this.entity = entity;
        this.user = user;
        setLayout(new BorderLayout());
        initTopPanels();
        initContentPanel("Debtors", "Customer");
        initDetailsPanel();
        setUpTimer(Global.debtors);
        initModels((Date)datePickerTicket.getModel().getValue(), Global.debtors);
        initGraphs(Global.debtors);
        setChartViewLine(Global.debtors.getDistribution((Date) datePickerTicket.getModel().getValue()));
        displayModel();
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        initModels((Date) d1.getValue(), Global.debtors);
        ...
        lineGraphPanel.refreshGraph(Global.debtors.getDistribution((Date)datePanelTicket.getModel().getValue()),0);
        ...
        if (isListView()){
            ...
            setChartViewBar(Global.debtors.getEntriesbyProductsModelGraph((Date) datePickerTicket.getModel().getValue()));
            detailsCon.setDetailsVisible(false);
        }
    }
