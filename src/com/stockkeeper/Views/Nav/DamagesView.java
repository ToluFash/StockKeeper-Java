...
public class DamagesView extends PrimaryViewSP implements MouseListener, ActionListener, DocumentListener, AdjustmentListener {
    //Core Models
    private Entity entity;
    private User user;

    public DamagesView(Entity entity, User user)
    {
        this.entity = entity;
        this.user = user;
        setLayout(new BorderLayout());
        initTopPanels();
        initContentPanel("Damages", "Customer");
        initDetailsPanel();
        setUpTimer(Global.damages);
        initModels((Date)datePickerTicket.getModel().getValue(),Global.damages);
        initGraphs(Global.damages);
        setChartViewLine(Global.damages.getDistribution((Date) datePickerTicket.getModel().getValue()));
        displayModel();
    ...
    private void initDetailsPanel(){
        ...
        detailsCon = new DamagesPanel(productLabel, qtyLabel, ticketIDLabel,ticketEntrySizeLabel,customerLabel,dateLabel);
        detailsCon.setDetailsVisible(false);
        detailsCon.addMouseListener(this);
        //Nav Area
        //Loading Components
        detailsPanel.add(infoBar, BorderLayout.NORTH);
        detailsPanel.add((DamagesPanel)detailsCon, BorderLayout.CENTER);
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        mouseClickedSP(e, "Returns Inward");
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        initModels((Date) d1.getValue(),Global.damages);
        ...
        lineGraphPanel.refreshGraph(Global.damages.getDistribution((Date)datePanelTicket.getModel().getValue()),0);
        ...
        if (isListView()){
            ...
            setChartViewBar(Global.damages.getEntriesbyProductsModelGraph((Date) datePickerTicket.getModel().getValue()));
            detailsCon.setDetailsVisible(false);
        }
    }
